...
float ThermalPlant_GetEquilibrium(float mv_percent)
{
    ...
    if (mv <= ID_MV_0) return 0.0f;
    if (mv <= ID_MV_20)
        return lerpf(ID_MV_0, 0.0f, ID_MV_20, ID_EQ_20, mv);
    if (mv <= ID_MV_50)
        return lerpf(ID_MV_20, ID_EQ_20, ID_MV_50, ID_EQ_50, mv);
    return lerpf(ID_MV_50, ID_EQ_50, ID_MV_80, ID_EQ_80, mv);
}
...
float ThermalPlant_GetTimeConstant(float mv_percent)
{
    ...
    if (mv <= ID_MV_20)
    {
        if (mv <= ID_MV_0) return DEFAULT_TAU_S;
        return lerpf(ID_MV_0, DEFAULT_TAU_S, ID_MV_20, ID_TAU_20, mv);
    }
    if (mv <= ID_MV_50)
        return lerpf(ID_MV_20, ID_TAU_20, ID_MV_50, ID_TAU_50, mv);
    if (mv <= ID_MV_80)
        return lerpf(ID_MV_50, ID_TAU_50, ID_MV_80, ID_TAU_80, mv);
    return ID_TAU_80;
}
                       ...
                       float sample_time_s)
{
    ...
    plant->sample_time_s = clampf(sample_time_s, MIN_SAMPLE_TIME_S, MAX_SAMPLE_TIME_S);
                        ...
                        float mv_percent)
{
    float equilibrium_c;
    float alpha;
    if ((plant == NULL) || !plant->initialized) return 0.0f;

    plant->mv_percent = clampf(mv_percent, PLANT_MIN_MV, PLANT_MAX_MV);
    plant->tau_s = clampf(ThermalPlant_GetTimeConstant(plant->mv_percent), MIN_TAU_S, 1000.0f);
    ...
    plant->temperature_c += alpha * (equilibrium_c - plant->temperature_c);
