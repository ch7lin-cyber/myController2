...
static void Fuzzy_LoadDefaultSet(FuzzyMembershipSet_t *set)
{
    ...
    /* Continuous partition. Each adjacent pair has non-zero overlap. */
    ...
    set->MF[FUZZY_NM].Center = -0.75f;
    set->MF[FUZZY_NM].Right  = -0.50f;

    set->MF[FUZZY_NS].Type   = FUZZY_MF_TRIANGLE;
    set->MF[FUZZY_NS].Left   = -0.75f;
    set->MF[FUZZY_NS].Center = -0.50f;
    set->MF[FUZZY_NS].Right  = -0.25f;

    set->MF[FUZZY_ZE].Type   = FUZZY_MF_TRIANGLE;
    set->MF[FUZZY_ZE].Left   = -0.50f;
    set->MF[FUZZY_ZE].Center =  0.00f;
    set->MF[FUZZY_ZE].Right  =  0.50f;

    set->MF[FUZZY_PS].Type   = FUZZY_MF_TRIANGLE;
    set->MF[FUZZY_PS].Left   =  0.25f;
    set->MF[FUZZY_PS].Center =  0.50f;
    set->MF[FUZZY_PS].Right  =  0.75f;

    set->MF[FUZZY_PM].Type   = FUZZY_MF_TRIANGLE;
    set->MF[FUZZY_PM].Left   =  0.50f;
    set->MF[FUZZY_PM].Center =  0.75f;
...
static bool Fuzzy_ValidateFunction(const FuzzyMembershipFunction_t *mf)
{
    ...
    if ((mf->Left < FUZZY_INPUT_MIN) || (mf->Left > FUZZY_INPUT_MAX) ||
        ...
        (mf->Right < FUZZY_INPUT_MIN) || (mf->Right > FUZZY_INPUT_MAX))
        return false;
    switch (mf->Type)
    {
        case FUZZY_MF_TRIANGLE:
...
bool FB_FuzzyMembership_SetErrorMF(FB_FuzzyMembership_t *fb,
                                   uint8_t index, FuzzyMFType_t type,
...
bool FB_FuzzyMembership_GetErrorMF(const FB_FuzzyMembership_t *fb,
                                   uint8_t index, FuzzyMembershipFunction_t *mf)
{
...
bool FB_FuzzyMembership_Validate(const FuzzyMembershipSet_t *set)
{
    ...
    for (i = 0U; i < (FUZZY_MF_COUNT - 1U); ++i)
    {
        ...
        /* Equal endpoints are a zero-degree hole for these MF definitions. */
